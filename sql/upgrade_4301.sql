ALTER TABLE  `civicrm_grant_app_page` ADD COLUMN IF NOT EXISTS `is_draft` TINYINT(4) DEFAULT NULL COMMENT 'Does this page have a Save as Draft button?' AFTER `default_amount`;

ALTER TABLE  `civicrm_grant_app_page` ADD COLUMN IF NOT EXISTS `draft_title` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT 'Title for Save as Draft page (header title tag, and display at the top of the page).' AFTER `is_draft`;

ALTER TABLE  `civicrm_grant_app_page` ADD COLUMN IF NOT EXISTS `draft_text` text COLLATE utf8_unicode_ci COMMENT 'Text and html allowed. Displayed above application fields on Save as Draft page' AFTER `draft_title`;

ALTER TABLE  `civicrm_grant_app_page` ADD COLUMN IF NOT EXISTS `draft_footer` text COLLATE utf8_unicode_ci COMMENT 'Text and html allowed. Displayed at the bottom of the Save as Draft page.' AFTER `draft_text`;

ALTER TABLE `civicrm_grant_app_page` ADD COLUMN IF NOT EXISTS `confirm_text` text COLLATE utf8_unicode_ci COMMENT 'Text and html allowed. displayed at the bottom of the confirmation page.' AFTER `thankyou_footer`, ADD `confirm_footer` text COLLATE utf8_unicode_ci COMMENT 'Text and html allowed. displayed at the bottom of the confirmation page.' AFTER `confirm_text`;
