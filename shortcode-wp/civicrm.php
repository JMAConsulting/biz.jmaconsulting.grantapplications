...
class CiviCRM_For_WordPress {
  ...
  */
  ...
  public function shortcode_handler( $atts ) {
    ...
    switch ( $component ) {
      ...
      case 'grant':
        $args['q'] = 'civicrm/grant/transact'; // + 17 dec    
      break;
 ...
 // + 17 dec 
  private function get_grant_pages() {
    $now = date('Ymdhis');
    $sql = "
        SELECT id, title
...
OR       ( start_date <= $now AND end_date >= $now )
    ...
    $grantAppPages = array();
    while ($dao->fetch()) {
      $grantAppPages[$dao->id] = $dao->title;
    }
    return $grantAppPages;
  }
          ...
          <div>
            ...
            <div style="padding:15px 15px 0 15px;">
              <select id="add_civicomponent_id">
                ...
                <option value="grant"><?php _e( 'CiviGrant', 'civicrm-wordpress' ); ?></option> <!-- // + 17 dec -->
              ...
              <br>
              <!-- // + 17 dec -->
              <span id="grant-section" style="display:none;">
                    <select id="add_grantapppage_id">
                            <?php
                              $grantAppPages = $this->get_grant_pages();
                              foreach ($grantAppPages as $key => $value) { ?>
                                <option value="<?php echo absint($key) ?>"><?php echo esc_html($value) ?></option>
                                <?php
                              }?>
                    </select>
