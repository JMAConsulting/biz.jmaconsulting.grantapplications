...
class CiviCRM_For_WordPress_Shortcodes_Modal {
  ...
  /**
   * Get ID and title of CiviCRM grant application pages
   *
   * @access private
   * @return array $grantAppPages Array of grant application pages
   */
  private function get_grant_app_pages() {
    $now = date('Ymdhis');
    $sql = "
        SELECT id, title
  ...
  public function add_form_button_html() {
    ...
    if ( $this->post_type_has_button() ) {
      ...
      $grant_app_pages = $this->get_grant_app_pages();
